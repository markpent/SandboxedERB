...
Processed Code
 email_address = user._sbm(:login) + "@" + user._sbm(:domain, :local_domain)
 
_smb will chech that the symbol of the function to call (:login and :domain) has been explicitly allowed for that object using the sandboxed_methods module function (example below).
