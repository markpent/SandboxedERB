...
2. The generated erb code is then processed to check that if conforms to the 'whitelist'of allowed syntax, as well as appending a 'to_sandbox' method onto every object (actually _sbm) to get a version of any object that is 
safe to use in the sandbox.
