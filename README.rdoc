...
* I use a 'white list' of allowed sytax, so if ive missed something it will be denied.
* I dont allow defining of any classes, methods or modules.
* You cannot access global variables or constants (or define them)
* It is assumed that any local variable is a 'sandboxed' object and that it is safe to call any method of it.
  * The return value of any method is converted to the sandboxed object automatically (code added during processing)
  * Object methods have been removed from sandboxed objects so no Kernel methods have been mixed in (__send__ etc)
